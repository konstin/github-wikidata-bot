...
from main import (Settings, analyse_release, get_all_github_releases, logger,
                  query_projects)


def safe_sample(pupulation, size):
    try:
        return sample(pupulation, size)
    except ValueError:
        return pupulation


def debug_version_handling(threshold: int = 50, size: int = 20):
    logger.setLevel(40)
    projects = safe_sample(query_projects(), threshold)
    for project in projects:
        github_releases = safe_sample(get_all_github_releases(project["repo"]), size)
...
if __name__ == "__main__":
    ...
    debug_version_handling(args.threshold, args.maxsize)
